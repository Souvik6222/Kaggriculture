"""Kaggriculture agent V5 - route replay with a public-state router.

WHAT THIS IS
    The season is not invented turn by turn. The agent replays a recorded 720-turn action
    stream from a strong ladder game, and at two points in the season decides which
    recorded continuation to follow, using only what the observation already shows.
    V5 keeps the proven V4 foundation byte-identical and adds three strictly
    order-preserving overlays (feed5, sparse SELL ordering, terminal $1 sweep).
...
THE TWO DECISIONS (public state only - no seed, no opponent identity, no future shop)
    t=226 (day  9.4)  a YARN_STORE has unlocked      -> wool-leaning tail
    t=360 (day 15.0)  carrot price >= 42             -> carrot tail   (only under yarn)
    (The old t=433 milk-inventory switch is removed: MAIN and MILK_GLUT are
    byte-identical until t=577, so deciding at t=433 commits 144 turns early
    on stale information.)

THE REPAIR LAYER
    V4 foundation (all preserved):
    ...
    V5 additions (order-preserving only - route BUY/HIRE/SELL slot positions
    never move, so the tape's cash-flow sequence is untouched):
      feed5      - on step 0, the tape's BUY_PRODUCT WHEAT order is kept in
                   market slot 0 (feed-denial defense; pure stable move).
      sell_order - SELLs are permuted only among slots already holding SELLs,
                   sorted by price*qty so the richest fills first. Non-SELL
                   slots never move, so no SELL ever jumps ahead of a BUY/HIRE
                   (the failure that cost -17 HOLD games for full reordering).
      terminal   - on steps 714-718 (718 is the last executed action; 719 never
                   runs) dead-stock also sells $1 goods instead of letting them
                   rot. Extra-only, so route sells are never evicted.
